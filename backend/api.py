...
async def shipment_plan(request:Request):
    ...
    plan=snapshot.shippable_plan
    if plan is None or plan.shippable_plan_id!=plan_id or plan.analysis_snapshot_id!=analysis_id:
        return error(409,'SHIPMENT_PLAN_PROVENANCE_MISMATCH','Shippable Plan does not belong to this analysis.','shippable_plan_id')
    if snapshot.source_mode is not SourceMode.API or plan.source_mode is not SourceMode.API:
        return error(409,'LIVE_VALIDATION_REQUIRES_API_SOURCE','Live validation requires an API-backed analysis.','analysis_snapshot_id')
    source_id=snapshot.source_snapshot_id
    source=OZON_SOURCE_STORE.get(source_id) if source_id else None
    if not source or source_id!=plan.source_snapshot_id or snapshot.analysis_as_of!=source.source_as_of or plan.analysis_as_of!=source.source_as_of:
        return error(409,'SHIPMENT_PLAN_PROVENANCE_MISMATCH','Analysis, plan, and source provenance do not match.','analysis_snapshot_id')
    try:scenario=parse_shipment_scenario(body.get('scenario'))
    except ValueError:return error(400,'INVALID_SHIPMENT_SCENARIO','Shipment scenario is invalid.','scenario')
    crossdock=any(method.value.endswith('crossdock') for method in scenario.allowed_methods)
    if crossdock:
        active={row.seller_warehouse_id for row in source.seller_warehouses if row.is_active}
        if scenario.seller_warehouse_id is not None and scenario.seller_warehouse_id not in active:return error(409,'SHIPMENT_PLAN_PROVENANCE_MISMATCH','Seller warehouse is no longer active.','scenario.seller_warehouse_id')
        if any(HANDOFF_STORE.get(x) is None or not HANDOFF_STORE.get(x).warehouse_type for x in scenario.selected_handoff_point_ids):return error(409,'SHIPMENT_PLAN_PROVENANCE_MISMATCH','Handoff point is unresolved.','scenario.selected_handoff_point_ids')
    try:OZON_VAULT.require_credentials()
    except OzonVaultError:return error(423,'OZON_VAULT_LOCKED','Unlock the Ozon credential vault first.',None)
    result=build_candidate_result(plan=plan,scenario=scenario,seller_warehouses=source.seller_warehouses,handoff_store=HANDOFF_STORE,max_candidates=DEFAULT_MAX_CANDIDATES)
    wanted=set(ids);by_id={x.candidate_id:x for x in result.candidates}
    if not wanted.issubset(by_id):return error(409,'SHIPMENT_PLAN_PROVENANCE_MISMATCH','Candidate does not belong to the stored scenario.','candidate_ids')
    selected=tuple(x for x in result.candidates if x.candidate_id in wanted)
    options=await asyncio.to_thread(DRAFT_VALIDATION_SERVICE.validate,selected,scenario,provenance=f'{analysis_id}:{plan_id}:{source_id}',source_clusters=source.clusters)
    try:shipment=build_shipment_plan(source_snapshot_id=source_id,analysis_snapshot_id=analysis_id,shippable_plan_id=plan_id,analysis_as_of=snapshot.analysis_as_of,scenario=scenario,candidates=selected,validations=options,diagnostics=tuple(x.code for x in result.diagnostics))
