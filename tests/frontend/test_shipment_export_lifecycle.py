...
def test_old_failure_cannot_invalidate_newer_plan():
    result = node(
        f"(()=>{{let s={CURRENT_PLAN};s={{...s,shipmentView:{{...s.shipmentView,plan:{{...s.shipmentView.plan,shipment_plan_id:'SHIP2'}}}}}};const next=SkladOzon.applyShipmentExportFailure(s,'SHIP1','OPTION1',{{code:'SHIPMENT_PLAN_NOT_FOUND'}});return {{same:next===s,id:next.shipmentView.plan.shipment_plan_id,dirty:next.shipmentView.dirty,code:next.shipmentView.planInvalidationCode,current:SkladOzon.isShipmentPlanCurrent(next)}};}})()"
    )
...
def test_export_response_acceptance_is_bound_to_current_plan():
    result = node(
        f"(()=>{{const current={CURRENT_PLAN};const stale=SkladOzon.invalidateShipmentPlan(current,'SHIPMENT_PLAN_NOT_FOUND');const newer={{...current,shipmentView:{{...current.shipmentView,plan:{{...current.shipmentView.plan,shipment_plan_id:'SHIP2'}}}}}};return {{stale:SkladOzon.canAcceptShipmentExportResponse(stale,'SHIP1'),old:SkladOzon.canAcceptShipmentExportResponse(newer,'SHIP1'),newer:SkladOzon.canAcceptShipmentExportResponse(newer,'SHIP2')}};}})()"
    )
...
def test_invalidation_preserves_in_flight_validation_identity():
    result = node(
        f"(()=>{{let s={CURRENT_PLAN};s={{...s,shipmentView:{{...s.shipmentView,runId:12,busyStage:'plan'}}}};return SkladOzon.applyShipmentExportFailure(s,'SHIP1','OPTION1',{{code:'SHIPMENT_PLAN_NOT_FOUND'}}).shipmentView;}})()"
    )
...
def test_app_wires_structured_failure_and_successful_recovery():
    ...
    for contract in ("plan:data.shipment_plan", "dirty:false", "planInvalidationCode:null", "exportErrors:{}"):
