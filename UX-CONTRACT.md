...
Раздел «Данные» содержит постоянный справочник кратности по артикулу: поиск по артикулу/SKU/названию, inline-редактирование, импорт и экспорт XLSX, а также сброс override к Unitka. Источник показывается как «Вручную», «Импорт», «Unitka» или «Не задано». Новое inline-значение считается сохранённым только после ответа backend; ошибки остаются рядом с действием.
